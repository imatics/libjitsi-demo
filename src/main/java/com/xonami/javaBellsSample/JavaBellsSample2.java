...
public class JavaBellsSample2 {
	...
	public static void main(String[] args) {
		...
		if( action == Action.CALL ) {
			try {
				m.startCall(JidCreate.bareFrom(m.receiverJid));
			} catch (Exception e1) {
				e1.printStackTrace();
			}
		}
	...
	public void startCall(Jid targetJid) throws Exception {
		...
		mainThread = new Thread() {
			...
			public void run() {
				try {
                    ...
                    connection.addStanzaSendingListener(stanza -> System.out.println( RECEIVER + " -----> : " + stanza.toXML() ), stanza -> true);
